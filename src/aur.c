...
alpm_list_t* exclude(alpm_list_t *excl){
	...
	do{
		/*
	 	 * if input is x-y interval, iterate and remove
	 	 * all packages with that number assigned
	 	 */
		if(sscanf(tok, "%d-%d", &s, &f) == 2){
			for(;f >= s; f--){
				aux = excl;
				while(aux){
					pkg = aux->data;
					if(pkg->num == f){
						excl = alpm_list_remove_item(excl, aux);
						break;
					}
					aux = alpm_list_next(aux);
				}
			}
		}
		/*
...
void install(aurpkg_t* pkg, alpm_list_t *t){
	...
	if(fork()){
		if(wait(NULL) == -1)
			fatal("An error occured which prevented makepkg from installing the package", t);
	}
	else{
		char path[BUFFER];
...
void aur_upgrade(alpm_list_t *to_upgrade, CURL *curl){
	...
	for(const alpm_list_t *t = to_upgrade; t; t = alpm_list_next(t)){
		 ...
		 * if directory did exist, pull
		 * check if you can't read PKGBUILD from rpc or using git before overwriting old PKGBUILD
		 */
		...
		if(!mkdir(path, 0755)){
			...
			fatal("Package directory doesn't already exist and could not be created.", alpm_list_next(t));	
			continue;
		}
	...
	while (t) {
		...
		if(config.filter & F_SYNC){
			for (alpm_list_t *p = pkgs; p; p = alpm_list_next (p)) {
				...
				/*
				 * all packages that are not due for an update are removed
				 * all that are due are assigned a number so they can be excluded if the user chooses to
				 */
