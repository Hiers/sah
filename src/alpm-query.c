...
static bool init_alpm (void)
{
	if (config.rootdir) {
		if (!config.dbpath) {
			char path[PATH_MAX];
			snprintf (path, PATH_MAX, "%s/%s", config.rootdir, DBPATH);
...
static bool parse_configfile (alpm_list_t **dbs, const char *configfile, bool reg)
{
	...
	if ((conf = fopen (configfile, "r")) == NULL) {
		fprintf (stderr, "Unable to open fileA: %s\n", configfile);
		return false;
	}
...
static bool filter (alpm_pkg_t *pkg, unsigned int _filter)
{
	...
	if ((_filter & F_SYNC) && !alpm_sync_get_new_version (pkg, alpm_get_syncdbs(config.handle)))
		return false;
...
static int filter_state (alpm_pkg_t *pkg)
{
	...
	if (filter (pkg, F_SYNC)) ret |= F_SYNC;
