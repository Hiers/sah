...
/*
 * User agent
 * Technically package-query will be the one interacting with the aur
 */
...
{	
	...
	if(config.op != OP_SEARCH)
		printf ("%s%d%s - ", color(C_NB), ++config.number, color(C_NO));
	...
	info = f (p, config.filter & F_SYNC ? 'V' : 'v');
	char *ver = STRDUP (info);
	info = (aur) ? f (p, 'm') : NULL;

	if (config.filter & F_SYNC) {
		printf ("%s%s%s -> %s%s%s", color(C_VER), lver, color(C_NO), color(C_VER), ver, color(C_NO));//prints diff to stdout
		
	} else {
		printf ("%s%s%s", color(C_VER), ver, color(C_NO));
	}
...
static int fetchhead_ref_cb( const char *name, const char *url, const git_oid *oid, unsigned int is_merge, void *payload){
	if(is_merge){
		memcpy(&branch_oid, oid, sizeof(git_oid));
		return 1;
	}
	return 0;
}

int fastforward_aur(git_repository *repo, git_annotated_commit *fetchhead_commit){
	git_reference *target_ref;
	git_object *target = NULL;
	const git_oid *target_oid = git_annotated_commit_id(fetchhead_commit);
	git_reference *new_target_ref;
	
	git_checkout_options checkout_options = GIT_CHECKOUT_OPTIONS_INIT;
	checkout_options.checkout_strategy = GIT_CHECKOUT_SAFE;

	if(git_repository_head(&target_ref, repo))
		return -1;
	
	if(git_object_lookup(&target, repo, target_oid, GIT_OBJECT_COMMIT))
		return -1;
	
	if(git_checkout_tree(repo, target, &checkout_options))
		return -1;
	
	if(git_reference_set_target(&new_target_ref, target_ref, target_oid, NULL))
		return -1;
...
int merge_aur(git_repository *repo, git_annotated_commit *fetchhead_commit, git_commit **parents){
	...
	if(git_merge(repo, (const git_annotated_commit**)&fetchhead_commit, 1, &merge_options, &checkout_options))
		return -1;
	...
	if(git_commit_lookup(&parents[1], repo, git_annotated_commit_id(fetchhead_commit)))
		return -1;
	

	git_signature_now(&sign, "sah", "sah@nomail.com");
	...
	if(git_commit_create(&commit_oid, repo, git_reference_name(head_ref), sign, sign, NULL, "Automated merge commit by sah.", tree, 2, 
...
int pull_aur(git_repository *repo){
	int r;

	git_remote *remote;
	git_strarray repo_array;
	git_annotated_commit *fetchhead_commit;
	git_merge_analysis_t analysis;
	git_merge_preference_t preference;

	/*
	 * steps are as a normal git pull, but we need to fetch a lot of info ourselves
	 */
	git_remote_list(&repo_array, repo);
	git_remote_lookup(&remote, repo, repo_array.strings[0]);
	git_fetch_options fetch_options = GIT_FETCH_OPTIONS_INIT;
	git_remote_fetch(remote, NULL, &fetch_options, NULL);

	/*
	 * for example, need to get head of branch to merge
	 */
	git_repository_fetchhead_foreach(repo, fetchhead_ref_cb, NULL);
	git_annotated_commit_lookup(&fetchhead_commit, repo, &branch_oid);

	if(git_merge_analysis(&analysis, &preference, repo, (const git_annotated_commit **)&fetchhead_commit, 1)){
		return -1;
	}
	...
	if(analysis & GIT_MERGE_ANALYSIS_UP_TO_DATE){
		...
		r = fastforward_aur(repo, fetchhead_commit);
	}

	else if(analysis & GIT_MERGE_ANALYSIS_NORMAL){
		git_commit **parents = (git_commit**)malloc(2*sizeof(git_commit*));
		r = merge_aur(repo, fetchhead_commit, parents);
		free(parents);
	}
	...
	git_strarray_dispose(&repo_array);
...
void fatal(char *mesg, void *left, ...){
	va_list arg;
	char buff[BUFFER];

	va_start(arg, left);
	vsnprintf(buff, BUFFER, mesg, arg);
	va_end(arg);
